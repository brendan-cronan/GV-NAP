...
class host1 extends JPanel {
	...
	private static final int SERVER_PORT = 6603, MY_PORT = 6607;
	...
	private void makeFileList() {
		...
		boolean fileExists;
		File localStorage = new File("./SharedFiles1");

		File fileList = new File("./SharedFiles1/FileList.txt");
		if (fileList.exists()) {
	...
	public void run() {
		...
		while (true) {
			try {
				...
				File readFile = new File("./SharedFiles1/" + requestedFile);
